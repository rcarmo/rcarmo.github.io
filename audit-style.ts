...
for(const file of files){const id=basename(file,".md");const raw=readFileSync(join(contentDir,file),"utf8");const{fm,body}=parseFrontmatter(raw);const ss=sections(body);const findings:Finding[]=[];const add=(severity:Severity,rule:string,text:string)=>findings.push({severity,rule,text});
 ...
 const proseRules:[string,RegExp,Severity][]=[
  ...
  ["acceptance-register",/\b(?:acceptance evidence|production evidence|reviewed evidence|completion gate|quality-gated|bounded[^\n]{0,80}release surface|deterministic parity evidence|Verified OpenAI Responses|evidence bundle)\b/gi,"warning"],
  ["process-register",/\b(?:in progress|being brought up to date|first public release|is being migrated)\b/gi,"warning"],
  ["american-spelling",/\b(?:behavior|behaviors|customization|optimized|optimizing|optimization|favorite|catalog|enrollment|summarization|visualization|normalization)\b/gi,"warning"],
  ["unicode-punctuation",/[—→“”‘’]/g,"warning"]];
 const proseBlocks=[...(fm.tagline?[{heading:"Tagline",content:fm.tagline}]:[]),...ss.filter(s=>proseSections.has(s.heading))];
 for(const sec of proseBlocks){const text=sec.content.replace(/<svg[\s\S]*/,"");
  for(const[rule,rx,severity]of proseRules)for(const m of text.matchAll(rx))add(severity,rule,`${sec.heading}: ${m[0]}`);
